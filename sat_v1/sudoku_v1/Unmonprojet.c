...
int k;
    ...
    while (i < 1 || i > dimension) {
        printf("Erreur : la valeur doit �tre supérieure ou égale à 1 et inférieure à %d \n ",dimension);
		printf("choisissez la ligne \n " );
		scanf("%d",&i);
    }
    if (i >= 1 && i <= dimension)
    {
        ...
        printf("Erreur : la valeur doit être supérieure ou égale à 1 et inférieure à %d ",dimension);
		printf("choisissez la colone \n ");
		scanf("%d",&j);
		}
        if (j >= 1 && j <= dimension){

        printf("la case choisie est %d,%d : \n" ,i,j);
        if(G[i-1][j-1] == 0) //seulement si case vide
            {
                //if(nombre_possibilite_case(G,i,j,dimension)>0){

                    printf("saisir une valeur à la case %d %d\n 0 si vous voulez laisser une case vide \n",i,j);
                    scanf("%d",&k);
...
void saisir_grille(Grille G, int dimension) {
	...
	printf("2 - Résoudre la grille : tapez 2\n");
	printf("3 - Remettre la grille à 0 : tapez 3\n");
